...
contract PropyClaimAddressV1 is ERC721, AccessControl, Ownable {

  using Counters for Counters.Counter;
  Counters.Counter private _tokenIdCounter;

  event TokenMinted(uint256 indexed tokenId, uint256 indexed tokenTier, string indexed tokenURI);
  event TokenTierUpdated(uint256 indexed tokenId, uint256 indexed tokenTier, string indexed tokenURI);
  event TokenURIUpdated(uint256 indexed tokenId, uint256 indexed tokenTier, string indexed tokenURI);
  ...
  bytes32 public constant TIER_UPDATER_ROLE = keccak256("TIER_UPDATER_ROLE"); // can manage tier of tokens -> 0xd01027b8b8f7adf2290189994849f6d73457160fa0538540fe521c4e0c13239d
  ...
  constructor(
    ...
    _setupRole(TIER_UPDATER_ROLE, _roleAdmin);
  ...
  modifier onlyTierUpdater() {
    require(hasRole(TIER_UPDATER_ROLE, msg.sender), "NOT_TIER_UPDATER");
    _;
  }
  ...
  modifier onlyTierAndTokenURIUpdater() {
    require(hasRole(TIER_UPDATER_ROLE, msg.sender), "NOT_STATUS_UPDATER");
    require(hasRole(TOKEN_URI_UPDATER_ROLE, msg.sender), "NOT_TOKEN_URI_UPDATER");
    _;
  }
  ...
  function mint(
    ...
    emit TokenMinted(_tokenIdCounter.current(), 1, _tokenURI);
  }
  ...
  function updateTokenTier(
    uint256 _tokenId,
    uint256 _tokenTier
  ) public onlyTierUpdater {
    require(_exists(_tokenId), "INVALID_TOKEN_ID");
    require(_tokenTier > 0, "NO_ZERO_TIER");
    Token storage token = tokens[_tokenId];
    token.tokenTier = _tokenTier;
    emit TokenTierUpdated(_tokenId, _tokenTier, token.tokenURI);
  }
  ...
  function updateTokenURI(
    ...
    emit TokenURIUpdated(_tokenId, token.tokenTier, _tokenURI);
  }

  function updateTokenTierAndURI(
    uint256 _tokenId,
    uint256 _tokenTier,
    string memory _tokenURI
  ) public onlyTierAndTokenURIUpdater {
    require(bytes(_tokenURI).length > 0, "EMPTY_TOKEN_URI");
    require(_exists(_tokenId), "INVALID_TOKEN_ID");
    require(_tokenTier > 0, "NO_ZERO_TIER");
    Token storage token = tokens[_tokenId];
    ...
    token.tokenTier = _tokenTier;
    token.tokenURI = _tokenURI;
    emit TokenTierUpdated(_tokenId, _tokenTier, _tokenURI);
    emit TokenURIUpdated(_tokenId, _tokenTier, _tokenURI);
  }
